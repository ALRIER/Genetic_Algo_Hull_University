...
# Canonical version with full `fits` vector return is in 07_fitness_objectives.R.
...
evolve_universal_estimator_per_family <- function(dist_name,
  ...
  if (identical(Sys.getenv("GA_SKIP_PERTURBATION", unset = "0"), "1")) {
    pert_ga <- list(fitness_mean = NA_real_, fitness_sd = NA_real_, fitness_q95 = NA_real_,
                    fitness_min = NA_real_, fitness_max = NA_real_)
    cat(sprintf("[PERTURB] %s | skipped by GA_SKIP_PERTURBATION=1\n", dist_name))
  } else {
...
evolve_universal_estimator_per_family_cv <- function(dist_name,
  ...
  export_cv_diagnostics <- function(results, best_fold_idx, scores, final_result = NULL) {
    if (is.null(run_dir) || !nzchar(run_dir)) return(invisible(FALSE))
    st <- stage_name %||% ss_tag
    st <- gsub("[^A-Za-z0-9]+", "_", st)
    safe_name <- function(s) gsub("[^A-Za-z0-9_]+", "_", as.character(s))
    prefix <- sprintf("%s_%s_seed%d", safe_name(dist_name), safe_name(st), as.integer(seed))
    fold_score_df <- data.frame(
      distribution = dist_name,
      stage = st,
      seed = as.integer(seed),
      fold = seq_along(scores),
      validation_score = as.numeric(scores),
      best_fold = seq_along(scores) == as.integer(best_fold_idx),
      stringsAsFactors = FALSE
    )
    safe_write_csv(fold_score_df, file.path(run_dir, sprintf("%s_FOLD_SCORES.csv", prefix)))

    conv_rows <- list()
    traj_rows <- list()
    for (kk in seq_along(results)) {
      rr <- results[[kk]]
      if (!is.null(rr$convergence) && is.data.frame(rr$convergence) && nrow(rr$convergence)) {
        cc <- rr$convergence
        cc$distribution <- dist_name
        cc$stage <- st
        cc$seed <- as.integer(seed)
        cc$fold <- kk
        cc$is_best_fold <- kk == as.integer(best_fold_idx)
        conv_rows[[length(conv_rows) + 1L]] <- cc
      }
      if (!is.null(rr$weight_traj) && is.data.frame(rr$weight_traj) && nrow(rr$weight_traj)) {
        tt <- rr$weight_traj
        tt$distribution <- dist_name
        tt$stage <- st
        tt$seed <- as.integer(seed)
        tt$fold <- kk
        tt$is_best_fold <- kk == as.integer(best_fold_idx)
        traj_rows[[length(traj_rows) + 1L]] <- tt
      }
    }
    if (!is.null(final_result$convergence) && is.data.frame(final_result$convergence) && nrow(final_result$convergence)) {
      cc <- final_result$convergence
      cc$distribution <- dist_name
      cc$stage <- st
      cc$seed <- as.integer(seed)
      cc$fold <- 0L
      cc$is_best_fold <- FALSE
      cc$is_final_retrain <- TRUE
      conv_rows[[length(conv_rows) + 1L]] <- cc
    }
    if (!is.null(final_result$weight_traj) && is.data.frame(final_result$weight_traj) && nrow(final_result$weight_traj)) {
      tt <- final_result$weight_traj
      tt$distribution <- dist_name
      tt$stage <- st
      tt$seed <- as.integer(seed)
      tt$fold <- 0L
      tt$is_best_fold <- FALSE
      tt$is_final_retrain <- TRUE
      traj_rows[[length(traj_rows) + 1L]] <- tt
    }
    if (length(conv_rows)) safe_write_csv(dplyr::bind_rows(conv_rows), file.path(run_dir, sprintf("%s_CONVERGENCE.csv", prefix)))
    if (length(traj_rows)) safe_write_csv(dplyr::bind_rows(traj_rows), file.path(run_dir, sprintf("%s_WEIGHT_TRAJECTORY.csv", prefix)))
    invisible(TRUE)
  }
  ...
  if (isTRUE(final_retrain)) {
    ...
    export_cv_diagnostics(fold_results, best_fold, val_scores, final_result = res_final)
...
# Override for the new experiment: summarize every registered estimator dynamically,
# including the expanded validation benchmark estimators added to ESTIMATOR_REGISTRY.
summarize_scenario <- function(S, weights, distribution, sample_size,
                               q95_B = 300L, q95_seed = NULL, q95_boot_indices = NULL) {
  if (exists("apply_estimator_admissibility", mode = "function", inherits = TRUE)) {
    weights <- apply_estimator_admissibility(weights, distribution = distribution, target = "arithmetic_mean")
  ...
  agg <- function(z) {
    z <- as.numeric(z); z <- z[is.finite(z)]
    if (length(z) == 0L) {
      return(c(mse=NA, mse_q95=NA, mse_max=NA, bias=NA, abs_bias=NA, variance=NA, mad=NA, iqr=NA))
    }
    errs2 <- (z - true_mu)^2
    c(
      mse      = base::mean(errs2, na.rm = TRUE),
  ...
  make_row <- function(est_name, m) dplyr::mutate(base_row, estimator = est_name, !!!as.list(m))
  if (is.null(C) || !is.matrix(C) || ncol(C) != N_EST) {
    na_metrics <- c(mse=NA_real_, mse_q95=NA_real_, mse_max=NA_real_, bias=NA_real_, abs_bias=NA_real_, variance=NA_real_, mad=NA_real_, iqr=NA_real_)
    return(dplyr::bind_rows(c(list(make_row("robust", na_metrics)), lapply(ESTIMATOR_NAMES, function(nm) make_row(nm, na_metrics)))))
  }
  w_robust <- .normalize_simplex(as.numeric(weights))
  est_robust <- as.vector(C %*% w_robust)
  rows <- c(list(make_row("robust", agg(est_robust))), lapply(seq_along(ESTIMATOR_NAMES), function(j) {
    make_row(ESTIMATOR_NAMES[j], agg(C[, j, drop = TRUE]))
  }))
