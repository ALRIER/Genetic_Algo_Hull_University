...
# ===== MODULE RUN TRACKING =====
# Keep the per-file load checks while remaining compatible with both full-project
# sourcing and standalone sourcing.
...
.allowed_benchmarks_for_distribution <- function(distribution = NULL) {
  ...
  if (exists("estimator_admissibility_report", mode = "function", inherits = TRUE)) {
    rep <- tryCatch(estimator_admissibility_report(distribution, target = "arithmetic_mean"),
...
.summarize_one_regime <- function(df, distribution = NULL,
  ...
  base <- data.frame(
    ...
    regime_key = if ("regime_key" %in% names(df)) as.character(df$regime_key[1]) else NA_character_,
    mechanism_regime = if ("mechanism_regime" %in% names(df)) as.character(df$mechanism_regime[1]) else NA_character_,
    rate_regime = if ("rate_regime" %in% names(df)) as.character(df$rate_regime[1]) else NA_character_,
    scale_regime = if ("scale_regime" %in% names(df)) as.character(df$scale_regime[1]) else NA_character_,
    sample_size_regime = if ("sample_size_regime" %in% names(df)) as.character(df$sample_size_regime[1]) else NA_character_,
    n_rows = nrow(df),
  ...
  regime_class <- if (isTRUE(gate_pass)) {
    "ga_promising"
  } else if (is.finite(rel_imp_q95) && is.finite(rel_imp_mean) &&
             rel_imp_q95 > -0.05 && rel_imp_mean > -0.05) {
    "ambiguous_near_benchmark"
  } else {
    "benchmark_dominant"
  }
  ...
  cbind(base, data.frame(
    ...
    gate_pass_regime = isTRUE(gate_pass),
    regime_class = regime_class,
    promise_score = as.numeric(promise_score),
    stringsAsFactors = FALSE
  ))
}

build_regime_discovery_summary <- function(scen_tbl,
                                           distribution = NULL,
                                           ga_estimator_name = "robust",
  ...
  if (nrow(out)) {
    out <- out |>
      dplyr::arrange(dplyr::desc(gate_pass_regime), dplyr::desc(promise_score), regime_key) |>
...
# Select regime-level candidates for a future specialist-training stage.
# This is still a reporting/selection layer: it does NOT alter GA training,
# halving, PSOCK, K-folds, or final benchmark gating. It creates an explicit
# list of regimes that should be considered for specialist GA optimization.
select_regimes_for_specialist_training <- function(regime_summary,
                                                   distribution = NULL,
                                                   top_k = 3L,
                                                   min_rows = 20L,
                                                   near_margin = 0.05,
                                                   holdout_frac = 0.30,
                                                   seed = 101L) {
  if (is.null(regime_summary) || !is.data.frame(regime_summary) || !nrow(regime_summary)) {
    return(data.frame())
  }
  top_k <- max(1L, as.integer(top_k)[1])
  min_rows <- max(1L, as.integer(min_rows)[1])
  near_margin <- as.numeric(near_margin)[1]
  if (!is.finite(near_margin) || near_margin < 0) near_margin <- 0.05
  ...
  need <- c("distribution", "regime_key", "n_rows", "regime_class",
            "promise_score", "ga_rel_improvement_q95", "ga_rel_improvement_mean",
            "ga_win_rate_vs_best_q95", "ga_q95_win_rate_vs_best_q95")
  for (nm in need) if (!nm %in% names(regime_summary)) regime_summary[[nm]] <- NA
  
  x <- regime_summary
  if (!is.null(distribution) && !"distribution" %in% names(x)) {
    x$distribution <- as.character(distribution)[1]
  }
  
  x <- x |>
    dplyr::mutate(
      enough_rows = is.finite(n_rows) & n_rows >= min_rows,
      ...
      eligible_for_specialist = enough_rows &
        (regime_class == "ga_promising" |
           (regime_class == "ambiguous_near_benchmark" & near_q95 & near_mean)),
      selection_priority = dplyr::case_when(
        prior_regime ~ 0L,
        regime_class == "ga_promising" ~ 1L,
        eligible_for_specialist & regime_class == "ambiguous_near_benchmark" ~ 2L,
        TRUE ~ 9L
      ),
      selection_reason = dplyr::case_when(
        regime_class == "ga_promising" ~ "passed_regime_gate",
        eligible_for_specialist & regime_class == "ambiguous_near_benchmark" ~ "near_benchmark_candidate",
        !enough_rows ~ "insufficient_regime_rows",
        TRUE ~ "not_selected_benchmark_dominant"
      )
    ) |>
    dplyr::arrange(selection_priority, dplyr::desc(opportunity_score), dplyr::desc(promise_score),
                   dplyr::desc(ga_rel_improvement_q95),
                   dplyr::desc(ga_rel_improvement_mean), regime_key) |>
    dplyr::group_by(distribution) |>
    dplyr::mutate(
      specialist_rank = dplyr::row_number(),
      selected_for_specialist_training = eligible_for_specialist & specialist_rank <= top_k,
  ...
  # Keep selected regimes first, but retain non-selected rows for auditability.
  x |>
    dplyr::arrange(dplyr::desc(selected_for_specialist_training),
                   selection_priority, specialist_rank, regime_key)
}

# =============================================================================
# MARTICA 2 — REGIME-AWARE SPECIALIST HELPERS WITH CONDITIONAL ELITE NEIGHBORHOOD
# =============================================================================
# These helpers are defined here to keep the release self-contained. They override
# earlier lightweight reporting helpers and add: explicit experimental-condition
# audit columns, true unique-scenario guards, specialist train/heldout splits,
# and conditional elite-neighborhood expansion with NA-safe fallbacks.
# =============================================================================
...
.summarize_one_regime <- function(df, distribution = NULL,
  ...
  # Opportunity score: HPF1/HPF2 should not only pass early GA
  # winners. They should also pass regimes where the GA is close to the best
...
select_regimes_for_specialist_training <- function(regime_summary, distribution = NULL,
  ...
  need <- c("distribution", "regime_key", "n_rows", "n_unique_scenarios", "regime_class", "promise_score",
            ...
            "source_stage", "source_config_index", "source_config_tag", "prior_regime", "external_prior_regime")
  ...
  x <- x |>
    dplyr::mutate(
      ...
      near_q95 = is.finite(ga_rel_improvement_q95) & ga_rel_improvement_q95 >= -near_margin,
      near_mean = is.finite(ga_rel_improvement_mean) & ga_rel_improvement_mean >= -near_margin,
      prior_regime = tolower(as.character(prior_regime)) %in% c("true", "1", "yes"),
      external_prior_regime = tolower(as.character(external_prior_regime)) %in% c("true", "1", "yes"),
      ...
      # earlier bug where Weibull-like sparse but promising regimes were discarded
      ...
      selection_reason = dplyr::case_when(
        prior_regime & !needs_neighborhood_for_eligibility ~ "external_prior_regime_exact_trainable",
        prior_regime & needs_neighborhood_for_eligibility ~ "external_prior_regime_pending_neighborhood",
...
.stage_elite_matrix <- function(elite_df, max_total = 12L) {
  ...
  clean_names <- sub("^w_", "", colnames(W))
  if (exists("ESTIMATOR_NAMES", inherits = TRUE) && exists("N_EST", inherits = TRUE)) {
    W_full <- matrix(0, nrow = nrow(W), ncol = N_EST)
    colnames(W_full) <- ESTIMATOR_NAMES
    matched <- match(clean_names, ESTIMATOR_NAMES)
    ok <- is.finite(matched) & !is.na(matched)
    if (!any(ok)) return(NULL)
    W_full[, matched[ok]] <- W[, ok, drop = FALSE]
    W <- W_full
  }
  W <- t(apply(W, 1, .normalize_simplex))
  W <- unique(W)
...
.write_candidate_provenance_table <- function(run_dir, family, seed, candidate_df,
                                              handoff_stage,
                                              inheritance_type = "internal_stage_elite",
                                              source_experiment_id = NA_character_,
                                              source_artifact = NA_character_) {
  if (is.null(run_dir) || !nzchar(run_dir)) return(invisible(NULL))
  if (is.null(candidate_df) || !is.data.frame(candidate_df) || !nrow(candidate_df)) return(invisible(NULL))
  wcols <- grep("^w_", names(candidate_df), value = TRUE)
  src <- function(nm, default = NA) if (nm %in% names(candidate_df)) candidate_df[[nm]] else default
  out <- data.frame(
    provenance_record_id = seq_len(nrow(candidate_df)),
    family = as.character(family)[1],
    seed = as.integer(seed)[1],
    handoff_stage = as.character(handoff_stage)[1],
    inheritance_type = as.character(inheritance_type)[1],
    source_experiment_id = as.character(source_experiment_id)[1],
    source_artifact = as.character(source_artifact)[1],
    candidate_source = as.character(src("candidate_source", NA_character_)),
    source_stage = as.character(src("source_stage", NA_character_)),
    source_config_index = suppressWarnings(as.integer(src("source_config_index", NA_integer_))),
    source_config_tag = as.character(src("source_config_tag", NA_character_)),
    source_score = suppressWarnings(as.numeric(src("source_score", NA_real_))),
    elite_rank_within_source = suppressWarnings(as.integer(src("elite_rank_within_source", NA_integer_))),
    distribution = as.character(src("distribution", family)),
    regime_key = as.character(src("regime_key", NA_character_)),
    regime_class = as.character(src("regime_class", NA_character_)),
    gate_pass_source = src("gate_pass", NA),
    q95_improvement_source = suppressWarnings(as.numeric(src("ga_rel_improvement_q95", NA_real_))),
    mean_improvement_source = suppressWarnings(as.numeric(src("ga_rel_improvement_mean", NA_real_))),
    protected_elite = TRUE,
    protected_elite_one_stage_only = TRUE,
    protection_rule = "Injected only into the next stage candidate pool; must compete under the next-stage objective/gate and is not declared a winner by inheritance.",
    bias_control_rule = "Prior/internal knowledge is used only as a warm-start/priority candidate; final evidence comes from heldout and fixed-weight validation without automatic pass-through.",
    provenance_interpretation = "This row documents candidate reuse so the workflow can audit provenance, selection pressure, and leakage risk.",
    stringsAsFactors = FALSE
  )
  if (length(wcols)) out <- cbind(out, candidate_df[, wcols, drop = FALSE])
  path <- file.path(run_dir, sprintf("PRIOR_CANDIDATE_PROVENANCE__seed%d.csv", as.integer(seed)[1]))
  old <- if (file.exists(path)) tryCatch(utils::read.csv(path, stringsAsFactors = FALSE), error = function(e) data.frame()) else data.frame()
  combined <- if (nrow(old)) dplyr::bind_rows(old, out) else out
  key_cols <- intersect(c("family", "seed", "handoff_stage", "inheritance_type", "candidate_source", "source_stage", "source_config_tag", "elite_rank_within_source", "regime_key"), names(combined))
  if (length(key_cols)) combined <- combined[!duplicated(combined[, key_cols, drop = FALSE]), , drop = FALSE]
  safe_write_csv(combined, path)
  invisible(path)
}


.read_prior_csvs <- function(files) {
  rows <- list()
  for (fp in files) {
    x <- tryCatch(utils::read.csv(fp, stringsAsFactors = FALSE), error = function(e) data.frame())
    if (!nrow(x)) next
    x$source_artifact <- normalizePath(fp, winslash = "/", mustWork = FALSE)
    x$source_experiment_id <- basename(dirname(dirname(fp)))
    rows[[length(rows) + 1L]] <- x
  }
  if (length(rows)) dplyr::bind_rows(rows) else data.frame()
}

.find_prior_files <- function(roots, family, pattern) {
  roots <- as.character(roots)
  roots <- roots[nzchar(roots) & dir.exists(roots)]
  if (!length(roots)) return(character(0))
  out <- character(0)
  for (rt in roots) {
    fps <- list.files(rt, pattern = pattern, recursive = TRUE, full.names = TRUE)
    if (length(fps)) {
      fps_norm <- normalizePath(fps, winslash = "/", mustWork = FALSE)
      out <- c(out, fps[grepl(paste0("/", family, "/"), fps_norm, fixed = TRUE)])
    }
  }
  unique(out)
}

.load_q1_prior_rows <- function(roots) {
  roots <- as.character(roots)
  roots <- roots[nzchar(roots) & dir.exists(roots)]
  rows <- list()
  for (rt in roots) {
    selected_fp <- file.path(rt, "root_summaries", "q1_selected_regimes.csv")
    final_fp <- file.path(rt, "tables", "q1_final_decision_table.csv")
    if (!file.exists(selected_fp) || !file.exists(final_fp)) next
    selected <- tryCatch(utils::read.csv(selected_fp, stringsAsFactors = FALSE), error = function(e) data.frame())
    final <- tryCatch(utils::read.csv(final_fp, stringsAsFactors = FALSE), error = function(e) data.frame())
    if (!nrow(selected) || !nrow(final) || !"validation_id" %in% names(selected) || !"validation_id" %in% names(final)) next
    keep_final <- intersect(c("validation_id", "final_q1_decision", "expanded_gate_pass_mean.locked_unseen_similar",
                              "ci_confirmed_mean.locked_unseen_similar", "mean_gain_mean.locked_unseen_similar",
                              "q95_gain_mean.locked_unseen_similar", "expanded_gate_pass_mean.original_regime",
                              "ci_confirmed_mean.original_regime", "mean_gain_mean.original_regime",
                              "q95_gain_mean.original_regime"), names(final))
    x <- merge(selected, final[, keep_final, drop = FALSE], by = "validation_id", all.x = TRUE)
    x$source_artifact <- normalizePath(selected_fp, winslash = "/", mustWork = FALSE)
    x$source_experiment_id <- basename(normalizePath(rt, winslash = "/", mustWork = FALSE))
    rows[[length(rows) + 1L]] <- x
  }
  if (length(rows)) dplyr::bind_rows(rows) else data.frame()
}

.q1_winner_filter <- function(x) {
  if (is.null(x) || !is.data.frame(x) || !nrow(x)) return(rep(FALSE, 0L))
  decision <- if ("final_q1_decision" %in% names(x)) tolower(as.character(x$final_q1_decision)) else rep("", nrow(x))
  grepl("ga_win$", decision) | decision %in% c("confirmed_ga_win", "accepted_ga_win", "marginal_ga_win")
}

.load_prior_hpf1_candidates <- function(family, roots, max_candidates = 12L) {
  q1 <- .load_q1_prior_rows(roots)
  x <- if (nrow(q1)) q1[as.character(q1$distribution) == as.character(family) & .q1_winner_filter(q1), , drop = FALSE] else data.frame()
  if (!nrow(x)) return(data.frame())
  wcols <- grep("^w_", names(x), value = TRUE)
  if (!length(wcols) && "ga_weight_vector" %in% names(x)) {
    parts <- strsplit(as.character(x$ga_weight_vector), ";", fixed = TRUE)
    max_len <- max(vapply(parts, length, integer(1)), 0L)
    if (max_len > 0L) {
      W <- t(vapply(parts, function(z) {
        zz <- suppressWarnings(as.numeric(z)); c(zz, rep(0, max_len - length(zz)))
      }, numeric(max_len)))
      for (jj in seq_len(ncol(W))) x[[paste0("w_prior", jj)]] <- W[, jj]
      wcols <- grep("^w_prior", names(x), value = TRUE)
    }
  }
  if (!length(wcols)) return(data.frame())
  q95 <- if ("q95_gain_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`q95_gain_mean.locked_unseen_similar`)) else if ("ga_rel_improvement_q95" %in% names(x)) suppressWarnings(as.numeric(x$ga_rel_improvement_q95)) else NA_real_
  mean_imp <- if ("mean_gain_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`mean_gain_mean.locked_unseen_similar`)) else if ("ga_rel_improvement_mean" %in% names(x)) suppressWarnings(as.numeric(x$ga_rel_improvement_mean)) else NA_real_
  q95[!is.finite(q95)] <- 0
  mean_imp[!is.finite(mean_imp)] <- 0
  ci <- if ("ci_confirmed_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`ci_confirmed_mean.locked_unseen_similar`)) else 0
  gate <- if ("expanded_gate_pass_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`expanded_gate_pass_mean.locked_unseen_similar`)) else 0
  x$prior_rank_score <- 1000 + 100 * ci + 50 * gate + 0.75 * q95 + 0.25 * mean_imp
  x <- x[order(-x$prior_rank_score), , drop = FALSE]
  x <- x[seq_len(min(nrow(x), max(1L, as.integer(max_candidates)[1]))), , drop = FALSE]
  if (any(grepl("^w_prior", names(x)))) {
    old <- grep("^w_prior", names(x), value = TRUE)
    new <- paste0("w_", if (exists("ESTIMATOR_NAMES", inherits = TRUE)) ESTIMATOR_NAMES[seq_along(old)] else seq_along(old))
    names(x)[match(old, names(x))] <- new
  }
  x$candidate_source <- "external_q1_winner"
  x$source_stage <- "Q1_FINAL_DECISION"
  x$protected_elite <- TRUE
  x$protected_elite_one_stage_only <- TRUE
  x$source_score <- x$prior_rank_score
  x$elite_rank_within_source <- seq_len(nrow(x))
  x
}

.load_prior_hpf1_regimes <- function(family, roots, max_regimes = 8L) {
  q1 <- .load_q1_prior_rows(roots)
  x <- if (nrow(q1)) q1[as.character(q1$distribution) == as.character(family) & .q1_winner_filter(q1), , drop = FALSE] else data.frame()
  if (!nrow(x) || !"regime_key" %in% names(x)) return(data.frame())
  if (!"distribution" %in% names(x)) x$distribution <- family
  x <- x[as.character(x$distribution) == as.character(family) | is.na(x$distribution), , drop = FALSE]
  if (!nrow(x)) return(data.frame())
  q95 <- if ("q95_gain_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`q95_gain_mean.locked_unseen_similar`)) else if ("ga_rel_improvement_q95" %in% names(x)) suppressWarnings(as.numeric(x$ga_rel_improvement_q95)) else NA_real_
  mean_imp <- if ("mean_gain_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`mean_gain_mean.locked_unseen_similar`)) else if ("ga_rel_improvement_mean" %in% names(x)) suppressWarnings(as.numeric(x$ga_rel_improvement_mean)) else NA_real_
  q95[!is.finite(q95)] <- 0
  ...
  ci <- if ("ci_confirmed_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`ci_confirmed_mean.locked_unseen_similar`)) else 0
  gate <- if ("expanded_gate_pass_mean.locked_unseen_similar" %in% names(x)) suppressWarnings(as.numeric(x$`expanded_gate_pass_mean.locked_unseen_similar`)) else 0
  x$prior_regime_score <- 1000 + 100 * ci + 50 * gate + 0.75 * q95 + 0.25 * mean_imp
  x <- x[order(-x$prior_regime_score), , drop = FALSE]
  x <- x[!duplicated(as.character(x$regime_key)), , drop = FALSE]
  x <- x[seq_len(min(nrow(x), max(1L, as.integer(max_regimes)[1]))), , drop = FALSE]
  if (!"n_unique_scenarios" %in% names(x)) x$n_unique_scenarios <- if ("n_exact_scenarios" %in% names(x)) suppressWarnings(as.integer(x$n_exact_scenarios)) else if ("n_scenarios" %in% names(x)) suppressWarnings(as.integer(x$n_scenarios)) else NA_integer_
  if (!"n_rows" %in% names(x)) x$n_rows <- if ("n_scenarios" %in% names(x)) suppressWarnings(as.integer(x$n_scenarios)) else x$n_unique_scenarios
  x$regime_class <- "external_q1_winner_regime"
  x$promise_score <- x$prior_regime_score
  x$opportunity_score <- x$prior_regime_score
  x$prior_regime <- TRUE
  x$external_prior_regime <- TRUE
  x$hpf_screen_stage <- "Q1_FINAL_DECISION"
  x$source_stage <- "Q1_FINAL_DECISION"
  x$source_config_index <- NA_integer_
  x$source_config_tag <- paste0("q1_final::", x$validation_id)
  x$source_seed <- if ("source_seed" %in% names(x)) suppressWarnings(as.integer(x$source_seed)) else NA_integer_
  x$source_lambda_instab <- NA_real_
  x$regime_rank <- seq_len(nrow(x))
  x
}

.write_prior_regime_provenance <- function(run_dir, family, seed, prior_rows) {
  if (is.null(prior_rows) || !is.data.frame(prior_rows) || !nrow(prior_rows)) return(invisible(NULL))
  keep <- intersect(c("distribution", "regime_key", "experimental_condition_key", "condition_summary", "regime_class",
                      "prior_regime_score", "ga_rel_improvement_q95", "ga_rel_improvement_mean", "promise_score",
                      "source_experiment_id", "source_artifact", "source_stage", "source_config_tag"), names(prior_rows))
  out <- prior_rows[, keep, drop = FALSE]
  out$family <- as.character(family)[1]
  out$seed <- as.integer(seed)[1]
  out$inheritance_type <- "external_prior_regime"
  out$handoff_stage <- "previous_experiment_to_HPF1_regime_pool"
  out$priority_rule <- "Prior regime is injected into the HPF1 discovery/ranking pool with priority but must survive specialist, heldout, and fixed-weight validation."
  out$bias_control_rule <- "No automatic win is granted; prior status is disclosed and final evidence is generated in the new experiment."
  safe_write_csv(out, file.path(run_dir, sprintf("PRIOR_REGIME_PROVENANCE__seed%d.csv", as.integer(seed)[1])))
  invisible(out)
}
...
run_regime_specialist_ga <- function(selected_regimes, dist_name, dist_param_grid, cfg,
  ...
  run_global_audit <- function(w, reg, reg_id, heldout_gate, heldout_rank, source_subset = NULL) {
    # Profile-matched fixed-weight audit.
...
run_regime_specialist_single_halving <- function(selected_regimes, dist_name, dist_param_grid, cfg,
  ...
  .write_candidate_provenance_table(
    run_dir = run_dir, family = dist_name, seed = as.integer(cfg$seed)[1],
    candidate_df = surv_log,
    handoff_stage = "specialist_halving_stage1_to_final",
    inheritance_type = "internal_specialist_halving_survivor",
    source_artifact = sprintf("REGIME_FIRST_SPECIALIST_HALVING_SELECTED__seed%d.csv", as.integer(cfg$seed)[1])
  )
...
run_regime_first_one_shot <- function(...) {
  ...
  .read_csv_recovery <- function(path) {
    if (!file.exists(path)) return(data.frame())
    tryCatch(utils::read.csv(path, stringsAsFactors = FALSE), error = function(e) data.frame())
  }
  ...
  .heartbeat_write <- function(root_out, stage, status = "running", family = NA_character_, detail = NA_character_, extra = list()) {
    if (is.null(root_out) || !nzchar(root_out)) return(invisible(NULL))
    ts <- format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z")
    pid <- Sys.getpid()
    elapsed <- if (exists(".run_t0", inherits = TRUE)) round(proc.time()[["elapsed"]] - get(".run_t0", inherits = TRUE), 2) else NA_real_
    detail <- if (is.null(detail)) NA_character_ else as.character(detail)
    family <- if (is.null(family)) NA_character_ else as.character(family)
    msg <- sprintf("[HEARTBEAT] %s | status=%s | stage=%s | family=%s | elapsed=%ss | %s", ts, status, stage, family, elapsed, ifelse(is.na(detail), "", detail))
    catf(msg)
    hb <- c(
      "{",
      sprintf('  "timestamp": "%s",', .json_escape(ts)),
      sprintf('  "pid": %d,', pid),
      sprintf('  "status": "%s",', .json_escape(status)),
      sprintf('  "stage": "%s",', .json_escape(stage)),
      sprintf('  "family": "%s",', .json_escape(family)),
      sprintf('  "elapsed_secs": %s,', ifelse(is.finite(elapsed), as.character(elapsed), "null")),
      sprintf('  "detail": "%s"', .json_escape(detail)),
      "}"
    )
    .atomic_write_lines(hb, file.path(root_out, "heartbeat.json"))
    row <- data.frame(timestamp = ts, pid = pid, status = status, stage = stage, family = family,
                      elapsed_secs = elapsed, detail = detail, stringsAsFactors = FALSE)
    hist_path <- file.path(root_out, "heartbeat_log.csv")
    if (file.exists(hist_path)) {
      utils::write.table(row, hist_path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE, qmethod = "double")
    } else {
      utils::write.csv(row, hist_path, row.names = FALSE)
    }
    invisible(row)
  }
  .stage_event <- function(root_out, stage, status, family = NA_character_, detail = NA_character_) {
    .heartbeat_write(root_out, stage = stage, status = status, family = family, detail = detail)
    row <- data.frame(timestamp = format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z"), stage = stage,
                      status = status, family = family, detail = detail, stringsAsFactors = FALSE)
    path <- file.path(root_out, "stage_recovery_ledger.csv")
    if (file.exists(path)) {
      utils::write.table(row, path, sep = ",", row.names = FALSE, col.names = FALSE, append = TRUE, qmethod = "double")
    } else {
      utils::write.csv(row, path, row.names = FALSE)
    }
    invisible(row)
  }
  .healthy_csv <- function(path, min_rows = 1L) {
    file.exists(path) && is.data.frame(.read_csv_recovery(path)) && nrow(.read_csv_recovery(path)) >= min_rows
  }
  ...
  resume_run_dir <- .get("resume_run_dir", Sys.getenv("GA_RESUME_RUN_DIR", unset = ""))
  recovery_enabled <- isTRUE(.get("recovery_enabled", TRUE))
  inherit_prior_winners <- isTRUE(.get("inherit_prior_winners", TRUE))
  default_prior_roots <- unique(c(
    file.path(getwd(), "Q1_Results_Final"),
    file.path(dirname(getwd()), "Q1_Results_Final")
  ))
  default_prior_roots <- default_prior_roots[dir.exists(default_prior_roots)]
  if (!length(default_prior_roots)) default_prior_roots <- file.path(getwd(), "Q1_Results_Final")
  prior_experiment_roots_raw <- .get(
    "prior_experiment_roots",
    Sys.getenv("GA_PRIOR_EXPERIMENT_ROOTS", unset = paste(default_prior_roots, collapse = ","))
  )
  prior_experiment_roots <- trimws(unlist(strsplit(paste(as.character(prior_experiment_roots_raw), collapse = ","), ",", fixed = TRUE)))
  prior_winner_max_per_family <- max(1L, as.integer(.get("prior_winner_max_per_family", 12L)))
  prior_regime_max_per_family <- max(1L, as.integer(.get("prior_regime_max_per_family", 8L)))
  ultra_fast_smoke <- isTRUE(.get("ultra_fast_smoke", FALSE))
  hp1_frac <- .get("hp1_frac", 0.25)
  hp1_k <- .get("hp1_k", NULL)
  hp1_gens <- max(if (ultra_fast_smoke) 1L else 3L, as.integer(.get("hp1_gens", 8L)))
  ...
  hp2_k <- .get("hp2_k", 8L)
  hp2_gens <- max(if (ultra_fast_smoke) 1L else 4L, as.integer(.get("hp2_gens", 15L)))
  ...
  if (recovery_enabled && nzchar(as.character(resume_run_dir)) && dir.exists(as.character(resume_run_dir))) {
    root_out <- normalizePath(as.character(resume_run_dir), winslash = "/", mustWork = TRUE)
    run_id <- basename(root_out)
    mkdirp(root_out)
    catf("[RECOVERY] Resuming existing run directory: %s", root_out)
  } else {
    run_id <- paste0("GA_REGIME_FIRST_", RUN_TS())
    root_out <- file.path(out_root, run_id); mkdirp(root_out)
  }
  .run_t0 <- proc.time()[["elapsed"]]
  .stage_event(root_out, "run", "started", detail = sprintf("recovery_enabled=%s resume_dir=%s", recovery_enabled, ifelse(nzchar(as.character(resume_run_dir)), as.character(resume_run_dir), "none")))
  index_init()
  ...
  meta <- list(
    ...
    note = "Global halving and global final eval are removed. HPF1 and HPF2 screen regime/config candidates; a single specialist-halving stage selects survivors; heldout benchmark gate selects final winners without retraining.",
    ...
    inherit_prior_winners = inherit_prior_winners,
    prior_experiment_roots = paste(prior_experiment_roots, collapse = ","),
    prior_winner_max_per_family = prior_winner_max_per_family,
    prior_regime_max_per_family = prior_regime_max_per_family,
    resume_run_dir = ifelse(nzchar(as.character(resume_run_dir)), as.character(resume_run_dir), NA_character_),
    heartbeat_json = file.path(root_out, "heartbeat.json"),
    heartbeat_log = file.path(root_out, "heartbeat_log.csv"),
    recovery_ledger = file.path(root_out, "stage_recovery_ledger.csv"),
  ...
  for (dist in families_to_run) {
    ...
    .stage_event(root_out, "family", "started", family = dist, detail = sprintf("dir=%s", fam_dir))
    recovered_winner_path <- file.path(fam_dir, sprintf("REGIME_FIRST_WINNER_SUMMARY__seed%d.csv", as.integer(seeds[1])))
    if (recovery_enabled && .healthy_csv(recovered_winner_path, min_rows = 1L)) {
      specialist_summary <- .read_csv_recovery(recovered_winner_path)
      .stage_event(root_out, "family", "skipped_recovered", family = dist, detail = sprintf("healthy winner summary exists: %s", recovered_winner_path))
      ga_wins <- sum(specialist_summary$gate_pass %in% TRUE | tolower(as.character(specialist_summary$gate_pass)) == "true", na.rm=TRUE)
      evaluated <- sum(specialist_summary$status == "evaluated", na.rm=TRUE)
      best_q95 <- suppressWarnings(max(as.numeric(specialist_summary$ga_rel_improvement_q95), na.rm=TRUE))
      best_mean <- suppressWarnings(max(as.numeric(specialist_summary$ga_rel_improvement_mean), na.rm=TRUE))
      if (!is.finite(best_q95)) best_q95 <- NA_real_
      if (!is.finite(best_mean)) best_mean <- NA_real_
      global_audit_runs <- if ("global_audit_run" %in% names(specialist_summary)) sum(specialist_summary$global_audit_run %in% TRUE | tolower(as.character(specialist_summary$global_audit_run)) == "true", na.rm=TRUE) else 0L
      global_audit_passes <- if ("global_audit_gate_pass" %in% names(specialist_summary)) sum(specialist_summary$global_audit_gate_pass %in% TRUE | tolower(as.character(specialist_summary$global_audit_gate_pass)) == "true", na.rm=TRUE) else 0L
      nfl_count <- if ("global_audit_interpretation" %in% names(specialist_summary)) sum(as.character(specialist_summary$global_audit_interpretation) == "regime_specific_no_free_lunch", na.rm=TRUE) else 0L
      global_bonus_count <- if ("global_audit_interpretation" %in% names(specialist_summary)) sum(as.character(specialist_summary$global_audit_interpretation) == "regime_and_global_bonus", na.rm=TRUE) else 0L
      ...
      winners_acc[[length(winners_acc)+1L]] <- data.frame(family=dist, architecture="regime_first", evaluated_regimes=evaluated,
        ...
        best_ga_rel_improvement_mean=best_mean, global_audit_runs=global_audit_runs, global_audit_passes=global_audit_passes,
        no_free_lunch_regime_specific_count=nfl_count, global_generalization_bonus_count=global_bonus_count,
        recovered_from_checkpoint=TRUE, stringsAsFactors=FALSE)
      runtime_rows[[length(runtime_rows)+1L]] <- data.frame(family=dist, phase="regime_first_family_recovered", elapsed_secs=round(proc.time()[["elapsed"]]-t_family,2),
        n_configs_eval=0L, n_scenarios=NA_integer_, stringsAsFactors=FALSE)
      next
    }
    sc_u_light <- add_scenario_ids(build_scenarios("light"))
    sc_u_full  <- add_scenario_ids(build_scenarios("full"))

    prior_hpf1_candidates <- if (isTRUE(inherit_prior_winners)) {
      .load_prior_hpf1_candidates(dist, prior_experiment_roots, max_candidates = prior_winner_max_per_family)
    } else data.frame()
    prior_hpf1_matrix <- .stage_elite_matrix(prior_hpf1_candidates, max_total = prior_winner_max_per_family)
    prior_regime_rows <- if (isTRUE(inherit_prior_winners)) {
      .load_prior_hpf1_regimes(dist, prior_experiment_roots, max_regimes = prior_regime_max_per_family)
    } else data.frame()
    if (nrow(prior_hpf1_candidates)) {
      .stage_event(root_out, "external_prior_winners", "loaded", family = dist, detail = sprintf("candidate_rows=%d", nrow(prior_hpf1_candidates)))
      .write_candidate_provenance_table(
        run_dir = fam_dir, family = dist, seed = as.integer(seeds[1]),
        candidate_df = prior_hpf1_candidates,
        handoff_stage = "previous_experiment_to_HPF1",
        inheritance_type = "external_prior_winner",
        source_experiment_id = paste(prior_experiment_roots, collapse = ","),
        source_artifact = "Q1_Results_Final/root_summaries/q1_selected_regimes.csv + Q1_Results_Final/tables/q1_final_decision_table.csv"
      )
    } else if (isTRUE(inherit_prior_winners)) {
      .stage_event(root_out, "external_prior_winners", "missing", family = dist, detail = "no prior winner rows found for family")
    }
    if (nrow(prior_regime_rows)) {
      .stage_event(root_out, "external_prior_regimes", "loaded", family = dist, detail = sprintf("regime_rows=%d", nrow(prior_regime_rows)))
      .write_prior_regime_provenance(fam_dir, dist, as.integer(seeds[1]), prior_regime_rows)
    } else if (isTRUE(inherit_prior_winners)) {
      .stage_event(root_out, "external_prior_regimes", "missing", family = dist, detail = "no prior regime rows found for family")
    }
    ...
    .stage_event(root_out, "RF_HPF1", "started", family = dist, detail = sprintf("configs=%d gens=%d ns=%d B=%d", nrow(fam_pool_phase1), hp1_gens, hp1_num_samples, hp1_bootstrap_B))
    ...
    for (i in seq_len(nrow(fam_pool_phase1))) {
      ...
      res1 <- evolve_universal_estimator_per_family_cv(
        ...
        protected_elite_matrix = prior_hpf1_matrix)
    ...
    .stage_event(root_out, "RF_HPF1", "completed", family = dist, detail = sprintf("configs_evaluated=%d", nrow(df1)))
    ...
    .write_candidate_provenance_table(
      run_dir = fam_dir, family = dist, seed = as.integer(seeds[1]),
      candidate_df = hpf1_stage_elites,
      handoff_stage = "HPF1_to_HPF2",
      inheritance_type = "internal_stage_elite",
      source_artifact = sprintf("REGIME_FIRST_HPF1_STAGE_ELITES__seed%d.csv", as.integer(seeds[1]))
    )
    hpf1_protected_matrix <- .stage_elite_matrix(hpf1_stage_elites, max_total = stage_elite_max_hpf1_to_hpf2)

    .stage_event(root_out, "RF_HPF2", "started", family = dist, detail = sprintf("configs=%d gens=%d ns=%d B=%d scenario_frac=%.2f", nrow(fam_pool), hp2_gens, hp2_num_samples, hp2_bootstrap_B, hp2_scenario_frac))
    ...
    .stage_event(root_out, "RF_HPF2", "completed", family = dist, detail = sprintf("configs_evaluated=%d", nrow(df2)))
    ...
    if (!is.null(hpf2_stage_elites) && nrow(hpf2_stage_elites)) {
      ...
      .write_candidate_provenance_table(
        run_dir = fam_dir, family = dist, seed = as.integer(seeds[1]),
        candidate_df = hpf2_stage_elites,
        handoff_stage = "HPF2_to_specialist_pool",
        inheritance_type = "internal_stage_elite",
        source_artifact = sprintf("REGIME_FIRST_HPF2_STAGE_ELITES__seed%d.csv", as.integer(seeds[1]))
      )
    }
    disc_all <- dplyr::bind_rows(hpf1_disc_all, hpf2_disc_all, prior_regime_rows)
    ...
    disc_all <- disc_all |>
      dplyr::mutate(
        prior_regime = if ("prior_regime" %in% names(disc_all)) tolower(as.character(prior_regime)) %in% c("true", "1", "yes") else FALSE,
        external_prior_regime = if ("external_prior_regime" %in% names(disc_all)) tolower(as.character(external_prior_regime)) %in% c("true", "1", "yes") else FALSE,
        hpf_stage_priority = dplyr::case_when(source_stage == "Q1_FINAL_DECISION" ~ 3L,
                                              source_stage == "RF_HPF2" ~ 2L,
                                              source_stage == "RF_HPF1" ~ 1L,
                                              TRUE ~ 0L)
      ) |>
      dplyr::arrange(dplyr::desc(prior_regime), dplyr::desc(gate_pass_regime), dplyr::desc(promise_score),
    ...
    if (!is.null(hpf2_protected_for_specialist) && nrow(hpf2_protected_for_specialist)) {
      ...
      .write_candidate_provenance_table(
        run_dir = fam_dir, family = dist, seed = as.integer(seeds[1]),
        candidate_df = hpf2_protected_for_specialist,
        handoff_stage = "HPF2_selected_regime_to_specialist_training",
        inheritance_type = "internal_stage_elite",
        source_artifact = sprintf("REGIME_FIRST_HPF2_PROTECTED_ELITES_FOR_SPECIALIST__seed%d.csv", as.integer(seeds[1]))
      )
    }
    ...
    .stage_event(root_out, "specialist_halving", "started", family = dist, detail = sprintf("selected_regimes=%d", sum(selected$selected_for_specialist_training %in% TRUE, na.rm=TRUE)))
    ...
    if (!is.null(specialist_summary) && nrow(specialist_summary)) {
      ...
      .stage_event(root_out, "specialist_halving", "completed", family = dist, detail = sprintf("winner_summary=%s rows=%d", out_sum, nrow(specialist_summary)))
    ...
    .stage_event(root_out, "family", "completed", family = dist, detail = sprintf("elapsed_secs=%.2f", proc.time()[["elapsed"]]-t_family))
  }
  progress_finalize()
  .stage_event(root_out, "run", "finalizing", detail = "binding family outputs")
  ...
  .bind_rows_relaxed <- function(rows) {
    rows <- Filter(function(z) is.data.frame(z) && nrow(z), rows)
    if (!length(rows)) return(data.frame())
    cols <- unique(unlist(lapply(rows, names), use.names = FALSE))
    for (cc in cols) {
      present <- vapply(rows, function(z) cc %in% names(z), logical(1))
      cls <- unique(vapply(rows[present], function(z) class(z[[cc]])[1], character(1)))
      if (length(cls) > 1L) {
        numericish <- all(cls %in% c("integer", "numeric"))
        for (ii in which(present)) {
          rows[[ii]][[cc]] <- if (numericish) suppressWarnings(as.numeric(rows[[ii]][[cc]])) else as.character(rows[[ii]][[cc]])
        }
      }
    }
    dplyr::bind_rows(rows)
  }
  audit_all <- if (length(global_audit_acc)) .bind_rows_relaxed(global_audit_acc) else data.frame()
  if (nrow(audit_all)) {
  ...
  .stage_event(root_out, "run", "completed", detail = sprintf("out_dir=%s", root_out))
...
if (RUN_MODE == "micro") {
...
} else if (RUN_MODE == "two_min_smoke") {
  # ============================================================================
  # TWO-MINUTE SMOKE — ultra-fast structural check for the new experiment.
  # This is not scientific evidence. It only verifies that the expanded registry,
  # HPF1/HPF2, specialist-halving wiring, and output exports can run end-to-end.
  # ============================================================================
  catf("RUN_MODE=two_min_smoke | ultra-fast structural check | OUT=%s", OUT_ROOT)

  out_smoke <- run_all_one_shot(
    families_to_run      = c("normal"),
    pop_sizes            = c(6),
    mutation_rates       = c(0.15),
    init_alphas          = c(0.7),
    alpha_mut_set        = c(0.7),
    immigrant_rates      = c(0.00),
    t_sizes              = c(2L),
    elitism_set          = c(1L),
    seeds                = c(101),
    sample_sizes         = c(300),
    num_samples          = 2L,
    generations_per_fold = 1L,
    k_folds              = 2L,
    objective            = "mixed",
    mix_w_q95            = 0.7,
    mix_w_max            = 0.3,
    ...
    min_delta            = 0.10,
    use_parallel         = FALSE,
    out_root             = file.path(OUT_ROOT, "two_min_smoke"),
    suppress_intermediate_saves = FALSE,
    hp_strict            = TRUE,
    hp1_frac             = 1.00,
    hp1_k                = 1L,
    ...
    hp1_num_samples      = 2L,
    hp1_bootstrap_B      = 2L,
    hp1_minibatch_frac   = 1.00,
    hp1_minibatch_min    = 2L,
    hp1_scenario_frac    = 0.10,
    hp2_k                = 1L,
    hp2_gens             = 1L,
    hp2_num_samples      = 2L,
    hp2_bootstrap_B      = 2L,
    hp2_minibatch_frac   = 1.00,
    hp2_minibatch_min    = 2L,
    hp2_scenario_frac    = 0.10,
    regime_top_k_per_family = 1L,
    regime_min_rows      = 1L,
    regime_near_margin   = 0.50,
    regime_holdout_frac  = 0.30,
    regime_min_train_scenarios = 1L,
    regime_min_heldout_scenarios = 1L,
    regime_specialist_finalists_per_regime = 1L,
    specialist_halving_stage1_budget_frac = 0.50,
    specialist_halving_keep_frac = 0.50,
    specialist_halving_min_keep = 1L,
    regime_neighborhood = TRUE,
    regime_neighborhood_trigger_scenarios = 2L,
    regime_neighborhood_max_neighbors = 1L,
    regime_neighborhood_max_scenarios = 4L,
    regime_global_audit = FALSE,
    regime_interfamily_profile_audit = FALSE,
    ultra_fast_smoke = TRUE,
    parallel_max_workers = 1L
  )

  print(out_smoke)
  catf("Two-minute smoke complete. Check: %s", normalizePath(file.path(OUT_ROOT, "two_min_smoke"), winslash="/", mustWork=FALSE))

} else if (RUN_MODE == "quick") {
  # ============================================================================
  # QUICK — smoke test on Windows (or any single machine)
  ...
  #
  #   SINGLE-NODE (Windows, or Linux without CLUSTER_HOSTS):
