...
# =============================================================================
# Enable debugging with DEBUG_TOPK=1 in the process environment.
...
# the execution path (early-stopping, error handling, etc.).
...
.git_info <- function() {
  ...
  tryCatch({
    hash <- suppressWarnings(system2("git", c("rev-parse", "--short", "HEAD"), stdout = TRUE, stderr = FALSE))
    br   <- suppressWarnings(system2("git", c("rev-parse", "--abbrev-ref", "HEAD"), stdout = TRUE, stderr = FALSE))
    if (length(hash) && length(br) && nzchar(hash[1]) && nzchar(br[1])) sprintf("%s (%s)", hash[1], br[1]) else NA_character_
