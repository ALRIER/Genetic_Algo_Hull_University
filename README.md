...
The repository is not an R package. It is a reproducibility and publication-companion record: code is organized by experimental stage, the committed result files provide the main tables, figures, and audit summaries needed to inspect the findings, and the publication appendices provide extended material referenced by the associated manuscript.
...
├── .zenodo.json
├── .gitignore
├── Publication_Appendices/
│   ├── Repository_Appendix_D_Detailed_Results.pdf
│   └── Repository_Appendix_E_Extended_Interpretation_and_Limitations.pdf
├── code/
...
└── scripts/                                  # archival and Zenodo preparation utilities
```

`code/` holds the source for each experimental stage. `results/` holds summary tables, diagnostic figures, evidence-taxonomy tables, run metadata, report files, and audit-ready CSV extracts used to inspect the main findings. `results/RESULTS_SELECTION_INDEX.csv` lists the committed result artifacts. `Publication_Appendices/` contains the extended publication material described below. Large raw outputs (RDS checkpoints, per-task dumps, recovery checkpoints, and downloaded public datasets) are not committed here; they belong in the separate full-data archive.

## Publication Appendices

The journal submission is accompanied by an official supplementary document containing **Supplementary Appendices A–C**, which provide the core methodological definitions, validation rules, audit procedures, and supporting tables required for peer review.

Two additional, extended appendices are versioned in this repository:

- [Repository Appendix D — Detailed Results](Publication_Appendices/Repository_Appendix_D_Detailed_Results.pdf)  
  Provides the expanded narrative of discovery, fixed-weight confirmation, evidence classification, audit findings, and external-validation results.

- [Repository Appendix E — Extended Interpretation and Limitations](Publication_Appendices/Repository_Appendix_E_Extended_Interpretation_and_Limitations.pdf)  
  Provides extended interpretation, deployment considerations, limitations, and the future research agenda.

These repository appendices complement the manuscript and the official Supplementary Appendices A–C. They do not replace the source code, committed result tables, or audit files used for reproducibility. For a stable citation, readers should use the archived repository release associated with the manuscript.
...
This repository holds source code, a summary layer of result files, and the two extended publication appendices stored in `Publication_Appendices/`. The official Supplementary Appendices A–C accompany the journal submission. Full Monte Carlo output (large CSV/RDS archives, per-task checkpoints, downloaded external datasets, and complete evidence packs) can exceed practical GitHub limits and should be stored in a separate research-data deposit.

## Citation

If you use this code or the repository appendices, please cite the associated research output and the archived release of this repository. When referring to Appendix D or E directly, include the appendix title and the repository release or DOI in the citation.
